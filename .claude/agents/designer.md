...
**프롬프트에 실려 온 숫자·표·인용과 `analysis.md`의 내용이 다르면 파일이 맞다.**
프롬프트로 받은 보고서는 파일을 손으로 옮겨 적은 요약이라 옮기다 틀릴 수 있다.
기준선 숫자처럼 **설계가 기대는 값은 반드시 `analysis.md`에서 다시 읽고**, 다르면
그 사실을 보고서에 적는다. (`finalizer.md`에도 같은 취지의 방어가 이미 있다 —
"프롬프트의 `판정: 통과` 한 줄을 믿지 마라. `review.md`를 직접 읽어라")
...
`사용자가 말한 이유:`는 왜 이 안을 골랐는지이고 → "채택 이유"에 쓴다.
`사용자가 덧붙인 말:`은 안을 고르면서 곁들인 추가 요청·주의사항·제약처럼 이유는 아니지만
설계에 반영해야 하는 말이다 → "핵심 결정"이나 관련 항목에 반영하고, 반영한 곳이 어디인지
보고서에 적는다. 둘 다 없으면 각각 "없음"으로 둔다.
