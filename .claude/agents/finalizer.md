...
- 판정이 `조건부통과`면, review.md의 "조건"을 읽고 **보고서에 그 조건을 그대로 적는다.**
  조건을 모른 채 커밋하지 마라.
  **판정의 근거는 항상 review.md다.** 프롬프트에도 `조건: <내용>` 줄이 실려 올 수 있는데,
  이건 오케스트레이터가 review.md를 보고 옮겨 적은 요약일 뿐이다. review.md의 "조건부 통과일 때의
  조건" 절과 내용이 같으면 그대로 쓰면 되고, **다르면 review.md 쪽을 따르고 둘이 달랐다는 사실을
  보고서에 적는다.** 프롬프트에 `조건:`이 없거나 비어 있어도 review.md에 조건이 있으면 그걸 쓴다 —
  프롬프트에 없다고 조건이 사라진 게 아니다.
...
- **archive** (프롬프트에 `archive: 해도 됨`이 있을 때만): **`openspec-archive-change` 절차를 따른다.**
...
`push: 해도 됨`과 짝이 맞는 필드다. **프롬프트에 `archive: 해도 됨`이 없으면** 사용자 승인이
전달되지 않은 것이니 아래 "승인 없을 때"로 간다. **있으면** 오케스트레이터가 사용자 승인을 받아
전달한 것이니 "승인 있을 때"로 간다.

**`openspec archive` CLI는 두 경우 모두 직접 돌리지 마라.** 이유:
...
**승인 없을 때 (기본):** archive는 **되돌릴 수 없는 일**이므로, 스킬이 사용자 확인을 요구하는
지점에서 **진행하지 말고 보고한다.** 확인할 것을 미리 조사해서 같이 올려라:
```bash
...
**승인 있을 때 (`archive: 해도 됨`):** 위 조사를 먼저 하고, 다음이 **전부** 맞을 때만
`openspec-archive-change` 절차를 끝까지 진행한다.
- `openspec validate "<이름>"`이 성공 (`exit=0`)
- 작업 목록에 `- [ ]` 미완료가 하나도 없음

하나라도 안 맞으면 승인이 있어도 **archive 하지 말고** 무엇이 막았는지 보고한다.
승인은 "확인 절차를 건너뛰라"는 뜻이 아니라 "확인이 통과하면 진행해도 된다"는 뜻이다.
스킬이 중간에 "Sync now / Archive without syncing / Cancel"을 물으면, 이 파이프라인은
sync를 먼저 하고 커밋하는 것이 원칙이므로 **Sync now에 해당하는 길로 간다** — 거기서 멈추지 마라.
...
- 프롬프트에 `archive: 해도 됨`이 없는데 archive 진행, `openspec archive` CLI 직접 실행
...
### archive
(승인 없을 때) 안 함 (되돌릴 수 없어서 사용자 확인 필요). 확인해 본 결과: 산출물 <상태> / validate <결과> / 미완료 작업 <개수>
(승인 있을 때 — `archive: 해도 됨`) 완료. 옮긴 경로: <archive 후 경로>. 확인해 본 결과: validate <결과> / 미완료 작업 <개수>
(승인 있었지만 조건 미달이었으면) 안 함 — 승인은 받았으나 <validate 실패/미완료 작업 있음>이라 진행하지 않음
```
